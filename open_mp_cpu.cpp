...
int main(int argc, char* argv[]) {
    ...
    for (int iter = 0; iter < max_iter; iter++) {
        ...
        // Etapa 2: recomputa centróides em paralelo
        vector<vector<double>> sum(K, vector<double>(D, 0.0));
        vector<int> count(K, 0);
        #pragma omp parallel
        {
            vector<vector<double>> sum_loc(K, vector<double>(D, 0.0));
            vector<int> count_loc(K, 0);
            #pragma omp for nowait
            for (int i = 0; i < N; i++) {
                int c = labels[i];
                count_loc[c]++;
                for (int d = 0; d < D; d++)
                    sum_loc[c][d] += data[i][d];
            }
            #pragma omp critical
            {
                for (int k = 0; k < K; k++) {
